...
/* async function cargarProductos() {
    try {
        const urlWebhook = 'http://localhost:5678/webhook/productos';
        ...
        // 1. Verificamos cómo llegó la información exactamente
        console.log("Datos crudos recibidos desde n8n:", data);
        
        let productosBase = [];
        
        // 2. Extraemos los productos sin importar cómo los envolvió n8n
        if (data.lista && Array.isArray(data.lista)) {
            productosBase = data.lista; // Si usamos ={{ $json }} sin el .lista en n8n
        } else if (Array.isArray(data)) {
            productosBase = data.map(item => item.json || item);
        } else if (data.json && data.json.lista) {
            productosBase = data.json.lista;
        } else {
            productosBase = [data];
        }
        
        console.log("Lista de productos extraída:", productosBase);
        
        misProductos = productosBase.map((producto, index) => {
            return { 
                ...producto,
                id: index + 1 
            };
        ...
        if (DOM.recipeGrid) {
            DOM.recipeGrid.innerHTML = '<p style="text-align:center; padding: 2rem;">Error al conectar con n8n.</p>';
        } 
    }
} */
 async function cargarProductos() {
...
const DOM = {
    ...
    checkoutBtn: document.getElementById('checkoutBtn'), // Agregado aquí para el checkout
};
...
// ==========================================
// 7. GESTIÓN DEL CARRITO DE COMPRAS Y PAGO SEGURO
// ==========================================
...
function actualizarCarritoUI() {
    ...
    if (DOM.cartItemsContainer) {
        if (carrito.length === 0) {
            DOM.cartItemsContainer.innerHTML = '<p class="empty-cart-text">Tu carrito espacial está vacío.</p>';
        } else {
            DOM.cartItemsContainer.innerHTML = carrito.map(item => `
                <div class="cart-item">
                    ...
                    <div class="cart-item-details">
                        ...
                        <span class="cart-item-price">$${item.precio.toLocaleString('es-AR')} x ${item.cantidad}</span>
                    </div>
...
async function procesarPagoSeguro() {
    const carritoActual = JSON.parse(localStorage.getItem('carritoCapibara')) || [];
    ...
    DOM.checkoutBtn.textContent = "Procesando orden...";
    DOM.checkoutBtn.disabled = true;

    // Simulamos una pequeña pausa estética y ejecutamos lo que pediste
    setTimeout(() => {
        // Mostramos el alert de redirección
        alert("Redirigiendo a pasarela de pagos...");
        
        // Limpiamos el carrito local y la UI
        localStorage.removeItem('carritoCapibara');
        carrito = []; 
        actualizarCarritoUI();
        cerrarPanelCarrito();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Cerrar carrito haciendo clic en el fondo oscuro
    DOM.cartOverlay?.addEventListener('click', (e) => {
        if (e.target === DOM.cartOverlay) cerrarPanelCarrito();
    });

    // EVENTO FALTANTE: Vincula el botón de "Finalizar Compra" con la función
    if (DOM.checkoutBtn) {
        DOM.checkoutBtn.addEventListener('click', procesarPagoSeguro);
    }
